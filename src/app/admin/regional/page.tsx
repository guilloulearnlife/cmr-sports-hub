'use client'
import { useState, useEffect } from 'react'
import Link from 'next/link'
import { Settings, RefreshCw, CheckCircle, Users, BarChart2, Shield, Smartphone } from 'lucide-react'
import { supabase } from '@/lib/supabase'

export default function AdminRegionalPage() {
  const [profile, setProfile] = useState<any>(null)
  const [stats, setStats] = useState({ enAttente: 0, clubs: 0, correspondants: 0 })

  useEffect(() => {
    supabase.auth.getSession().then(async ({ data }) => {
      if (!data.session) { window.location.href = '/admin/login'; return }
      const { data: p } = await supabase.from('profiles').select('*').eq('id', data.session.user.id).single()
      if (!p || !['admin_regional', 'super_admin'].includes(p.role)) {
        window.location.href = '/encoder'; return
      }
      setProfile(p)
      loadStats(p)
    })
  }, [])

  async function loadStats(p: any) {
    const [attente, clubs, corrs] = await Promise.all([
      supabase.from('matchs').select('id', { count: 'exact' }).eq('statut', 'en_attente'),
      supabase.from('clubs').select('id', { count: 'exact' }).eq('actif', true),
      supabase.from('profiles').select('id', { count: 'exact' }).eq('role', 'correspondant'),
    ])
    setStats({ enAttente: attente.count ?? 0, clubs: clubs.count ?? 0, correspondants: corrs.count ?? 0 })
  }

  async function deconnexion() {
    await supabase.auth.signOut()
    window.location.href = '/admin/login'
  }

  if (!profile) return (
    <div className="min-h-screen bg-dark flex items-center justify-center">
      <RefreshCw size={24} className="animate-spin text-cmr-yellow"/>
    </div>
  )

  const sections = [
    {
      title: 'Validation Scores',
      description: 'Approuver ou rejeter les scores soumis',
      href: '/admin/validation',
      icon: <CheckCircle size={28}/>,
      color: 'border-green-500 text-green-400',
      bg: 'hover:bg-green-900/20',
      badge: stats.enAttente > 0 ? stats.enAttente : null,
    },
    {
      title: 'Mes Clubs',
      description: 'Gerer les clubs de votre discipline',
      href: '/admin/clubs',
      icon: <Shield size={28}/>,
      color: 'border-purple-500 text-purple-400',
      bg: 'hover:bg-purple-900/20',
    },
    {
      title: 'Correspondants',
      description: 'Gerer vos correspondants et encodeurs',
      href: '/admin/utilisateurs',
      icon: <Users size={28}/>,
      color: 'border-orange-500 text-orange-400',
      bg: 'hover:bg-orange-900/20',
    },
    {
      title: 'Classements',
      description: 'Voir les classements de votre discipline',
      href: '/admin/classements',
      icon: <BarChart2 size={28}/>,
      color: 'border-red-500 text-red-400',
      bg: 'hover:bg-red-900/20',
    },
    {
      title: 'Planifier un Match',
      description: 'Creer un match et assigner un correspondant',
      href: '/admin/matchs',
      icon: <Settings size={28}/>,
      color: 'border-blue-500 text-blue-400',
      bg: 'hover:bg-blue-900/20',
    },
    {
      title: 'Encodage Direct',
      description: 'Encoder vous-meme un score en direct',
      href: '/admin/encoder',
      icon: <Smartphone size={28}/>,
      color: 'border-teal-500 text-teal-400',
      bg: 'hover:bg-teal-900/20',
    },
  ]

  return (
    <div className="min-h-screen bg-dark">
      <header className="border-b-2 border-cmr-yellow sticky top-0 z-50"
              style={{ background: 'linear-gradient(135deg, #0c2218, #091510)' }}>
        <div className="flag-strip"><div className="fg"/><div className="fr"/><div className="fy"/></div>
        <div className="max-w-screen-lg mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Settings size={18} className="text-cmr-yellow"/>
            <span className="font-oswald font-bold text-cmr-yellow tracking-widest">ADMIN REGIONAL</span>
            <span className="text-xs text-green-muted">{profile.email}</span>
          </div>
          <button onClick={deconnexion}
                  className="text-xs text-red-400 hover:text-red-300 font-oswald tracking-wider transition-colors border border-red-900 px-3 py-1 rounded hover:border-red-700">
            Deconnexion
          </button>
        </div>
      </header>

      <div className="max-w-screen-lg mx-auto px-4 py-8">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-10">
          {[
            { label: 'En attente validation', val: stats.enAttente, color: stats.enAttente > 0 ? 'text-red-400' : 'text-green-muted' },
            { label: 'Clubs', val: stats.clubs, color: 'text-purple-400' },
            { label: 'Correspondants', val: stats.correspondants, color: 'text-orange-400' },
          ].map(s => (
            <div key={s.label} className="card p-4 text-center">
              <div className={`font-oswald font-bold text-3xl ${s.color}`}>{s.val}</div>
              <div className="text-xs text-green-dim font-oswald tracking-wider mt-1">{s.label}</div>
            </div>
          ))}
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {sections.map(s => (
            <Link key={s.href} href={s.href}
                  className={`card p-6 border-l-4 ${s.color} ${s.bg} transition-all hover:scale-105 relative`}>
              {s.badge && (
                <span className="absolute top-3 right-3 bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">
                  {s.badge}
                </span>
              )}
              <div className="mb-3">{s.icon}</div>
              <div className="font-oswald font-bold text-white tracking-wider mb-1">{s.title}</div>
              <div className="text-xs text-green-muted">{s.description}</div>
            </Link>
          ))}
        </div>
      </div>
    </div>
  )
}
