...
export default function EncoderScorePage() {
  ...
  async function encoderScore(e: React.FormEvent) {
    ...
    if (error) {
      ...
      setMessage({ type: 'ok', text: 'Score encode !' })
    ...
    <div className="min-h-screen bg-dark p-6">
      <div className="max-w-screen-lg mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div>
            <h2 className="font-oswald text-xl tracking-widest text-white mb-4">Matchs a jouer ({matchs.length})</h2>
            <div className="space-y-2 max-h-96 overflow-y-auto">
              ...
              {matchs.map(m => (
                <div key={m.id} onClick={() => setScoreForm(p => ({ ...p, match_id: m.id }))}
                     className={`card p-3 cursor-pointer transition-all hover:border-green-mid ${scoreForm.match_id === m.id ? 'border-cmr-yellow' : ''}`}>
                  <div className="text-xs text-green-muted mb-1">{m.competition_nom} J{m.journee}</div>
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-sm">{m.dom_nom}</span>
                    <span className="text-xs text-green-muted">vs</span>
