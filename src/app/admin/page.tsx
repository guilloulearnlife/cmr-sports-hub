...
export default function AdminPage() {
  ...
  const [clubForm, setClubForm] = useState({
    slug:  '', nom: '', sigle: '', sport: 'football', ville: '',
    federation_id: '',
  })
  ...
  async function encoderScore(e: React.FormEvent) {
    ...
    if (error) {
      ...
      setMessage({ type: 'ok', text: '✅ Score encodé ! n8n va recalculer le classement automatiquement.' })

      // Appeler le recalcul via RPC Supabase
  ...
  async function creerMatch(e: React.FormEvent) {
    ...
    if (error) {
      ...
      setMessage({ type: 'ok', text: '✅ Match créé avec succès !' })
  ...
  return (
    <div className="min-h-screen bg-dark">
      ...
      <div className="max-w-screen-lg mx-auto px-4 py-8">
          ...
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
                    className={`flex items-center gap-2 px-4 py-3 font-oswald text-sm tracking-wider transition-all
                      ${tab === t.id
                        ? 'text-cmr-yellow border-b-2 border-cmr-yellow -mb-px'
                        : 'text-green-muted hover:text-white'}`}>
              {t.icon}{t.label}
            </button>
          ))}
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <form onSubmit={encoderScore} className="card p-6 space-y-4">
                ...
                <div>
                  ...
                  <select value={scoreForm.match_id}
                          ...
                          required
                          className="w-full bg-deep border border-border rounded px-3 py-2 text-sm text-white focus:border-cmr-yellow outline-none">
                    <option value="">-- Choisir un match --</option>
                    {matchs.map(m => (
                      <option key={m.id} value={m.id}>
                        {m.competition_nom} J{m.journee} · {m.dom_nom} vs {m.ext_nom}
                        {m.date_match ? ` (${new Date(m.date_match).toLocaleDateString('fr-FR')})` : ''}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" min="0" max="99"
                           value={scoreForm.score_domicile}
                           onChange={e => setScoreForm(p => ({ ...p, score_domicile: e.target.value }))}
                           required
                           className="w-full bg-deep border border-border rounded px-3 py-3 text-3xl font-oswald text-center text-white focus:border-cmr-yellow outline-none"
                           placeholder="0"/>
                  </div>
                  <div>
                    <label className="block text-xs text-green-muted font-oswald tracking-wider uppercase mb-2">
                    ...
                    <input type="number" min="0" max="99"
                           value={scoreForm.score_exterieur}
                  ...
                  {scoreForm.statut === 'en_direct' && (
                    <div>
                      ...
                      <input type="number" min="1" max="120"
                             ...
                             className="w-full bg-deep border border-border rounded px-3 py-2 text-white focus:border-cmr-yellow outline-none"
                             placeholder="ex: 67"/>
                    </div>
                  )}
                </div>
                ...
                <button type="submit" disabled={saving}
                  ...
                  {saving ? 'Enregistrement...' : 'Encoder le Score'}
                </button>
              </form>

              <p className="text-xs text-green-dim mt-3 text-center">
                ✨ Le classement est recalculé automatiquement après chaque score
              </p>
            </div>
            ...
            <div>
              <h2 className="font-oswald text-xl tracking-widest text-white mb-4">
                Matchs à jouer ({matchs.length})
              </h2>
              <div className="space-y-2 max-h-[500px] overflow-y-auto pr-1">
                {matchs.length === 0 && (
                  <div className="card p-8 text-center text-green-muted text-sm">Aucun match planifié</div>
                )}
                {matchs.map(m => (
                  <div key={m.id}
                       onClick={() => setScoreForm(p => ({ ...p, match_id: m.id }))}
                       className={`card p-3 cursor-pointer transition-all hover:border-green-mid
                         ${scoreForm.match_id === m.id ? 'border-cmr-yellow bg-cmr-yellow/5' : ''}`}>
                    <div className="text-xs text-green-muted font-oswald tracking-wider mb-1">
                      {m.competition_nom} · J{m.journee}
                    </div>
        ...
        {tab === 'clubs' && (
          <div className="max-w-lg">
            <h2 className="font-oswald text-xl tracking-widest text-white mb-4">Ajouter un club / équipe</h2>
            <form onSubmit={creerClub} className="card p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <FormField label="Sigle (3 lettres)">
              ...
              <FormField label="Slug URL (auto si vide)">
                <input type="text" value={clubForm.slug}
                       onChange={e => setClubForm(p=>({...p, slug: e.target.value.toLowerCase().replace(/\s/g,'-')}))}
                       className="form-input" placeholder="canon-yaounde"/>
              </FormField>
...
function FormField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-xs text-green-muted font-oswald tracking-wider uppercase mb-2">{label}</label>
      {children}
    </div>
  )
}
