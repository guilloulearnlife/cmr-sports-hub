                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6 gap-3">
                  {clubsList.map((c: any) => (
                    <div key={c.id} className="card p-4 text-center hover:border-green-mid transition-all">
                      <div className="w-12 h-12 rounded-full mx-auto mb-3 flex items-center justify-center font-oswald font-bold text-sm text-white border border-white/10"
                           style={{ background: c.couleur_maillot_dom ?? '#1a4a2e' }}>
                        {c.sigle ?? c.nom.slice(0,3).toUpperCase()}
                      </div>
